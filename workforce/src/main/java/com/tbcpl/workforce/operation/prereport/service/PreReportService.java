...
public class PreReportService {
    ...
    /**
     * Called by CaseService after a case is successfully created.
     * Updates prereport status to CASE_GENERATED and stores the case number.
     */
    @Transactional
    public void markCaseGenerated(Long prereportId, String caseNumber, Long caseId) {
        log.info("Marking prereport ID: {} as CASE_GENERATED with caseNumber: {}", prereportId, caseNumber);

        PreReport preReport = preReportRepository.findById(prereportId)
                .orElseThrow(() -> new RuntimeException("Pre-report not found with ID: " + prereportId));

        preReport.setReportStatus(ReportStatus.CASE_GENERATED);
        preReport.setCaseNumber(caseNumber);
        preReport.setCaseId(caseId);
        preReport.setUpdatedAt(LocalDateTime.now());

        preReportRepository.save(preReport);

        log.info("Prereport ID: {} successfully marked as CASE_GENERATED", prereportId);
    }
